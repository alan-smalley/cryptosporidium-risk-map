...
# 2. Data Loading & Coordinate Transformation
@st.cache_data
def load_and_prepare_data():
    # Load site metadata
    meta_df = pd.read_csv('zs_combined_(all)_REORDERED.csv')

    # Convert British National Grid (Eastings/Northings) to Lat/Lon
    ...
    # Load provisional predictions
    pred_df = pd.read_csv('compiled_site_adaptive_predictions_with_risk_levels.csv')

    # Handle missing risk levels cleanly
    ...
    # Merge model predictions with coordinates on 'Site'
    merged_df = pd.merge(
        pred_df, 
...
# Month Selector (Defaults to latest month in dataset: 2026-09)
...
with col_chart:
    ...
    # Line graph: Predicted vs Observed Risk
    fig_chart = go.Figure()
    ...
    # Show latest site metadata info card
    ...
    if not latest_site_row.empty:
        ...
        curr_model = latest_site_row['Selected_Model'].values[0]
